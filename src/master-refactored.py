...
def execute_streaming_split(dataset_name, dataset_variant, is_custom=False, custom_s3_url=None):
    config = load_config()
    ...
    if is_custom:
        # Read from given url
        _, source_key = parse_s3_uri(custom_s3_url)
        # Extract filename
        file_name = source_key.split('/')[-1].replace('.csv', '')

        # choose s3 custom dataset train and test set paths
...
def generate_inference_tasks(job_data, train_resp):
    ...
    if dataset == "custom":
        ...
        if 'test_s3_url' in job_data:
            test_s3_uri = job_data['test_s3_url']
        else:
            # Altrimenti (siamo nell'End-to-End), lo deduciamo dalla cartella splits/
            original_filename = job_data.get('custom_s3_url', '').split('/')[-1].replace('.csv', '')
            target_bucket = config.get("s3_bucket", "distributed-random-forest-bkt")
            test_s3_uri = f"s3://{target_bucket}/splits/{original_filename}_test.csv"
    else:
        target_bucket = config.get("s3_bucket", "distributed-random-forest-bkt")
...
def save_metrics(dataset, dataset_variant, n_workers, n_trees, strategy_name, train_time, inf_time, metrics_dict,
                 config):
...
def aggregate_and_evaluate(job_data, job_id, dataset_name, dataset_variant, s3_inference_results, num_workers, trees,
    ...
    if dataset_name == "custom":
        ...
        # Stessa logica di fallback intelligente
        if 'test_s3_url' in job_data:
            test_s3_uri = job_data['test_s3_url']
        else:
            original_filename = job_data.get('custom_s3_url', '').split('/')[-1].replace('.csv', '')
            test_s3_uri = f"s3://{config.get('s3_bucket')}/splits/{original_filename}_test.csv"
    else:
...
def process_training(job_data, job_id):
    """Gestisce l'intera logica di provisioning, split e addestramento distribuito."""
    dataset = job_data['dataset']
    num_workers = job_data['num_workers']
    ...
    # Questo flag arriverà dal Client (vedi spiegazione sotto)
    needs_split = job_data.get('needs_split', True)

    if not tasks_dispatched:
        if dataset == "custom":
            custom_s3_url = job_data['custom_s3_url']
            bucket, original_key = parse_s3_uri(custom_s3_url)

            if needs_split:
                print(" [PIPELINE] Custom Dataset requires splitting.")
                # Deduciamo dove andrà a finire il file di train
                file_name = original_key.split('/')[-1].replace('.csv', '')
                train_key = f"splits/{file_name}_train.csv"

                # Controlliamo se lo abbiamo già splittato in passato!
                if not check_s3_file_exists(bucket, train_key):
                    try:
                        calculated_train_rows, new_train_url = execute_streaming_split(
                            dataset, dataset_variant, is_custom=True, custom_s3_url=custom_s3_url
                        )
                        # Aggiorniamo l'URL: i worker devono leggere il file splittato, non quello originale!
                        job_data['custom_s3_url'] = new_train_url
                    except Exception as e:
                        print(f" [CRITICAL] Split failed: {e}")
                        return
                else:
                    print(" [PIPELINE] Split already exists for this custom dataset. Bypassing.")
                    calculated_train_rows = get_total_rows_s3_select(bucket, train_key)
                    job_data['custom_s3_url'] = f"s3://{bucket}/{train_key}"
            else:
                print(" [PIPELINE] User requested NO SPLIT. Using URL as raw Training Set.")
                try:
            ...
            # --- LOGICA PER DATASET GOLDEN STANDARD (Taxi/Airlines) ---
            bucket = config.get("s3_bucket")
            metadata = config['datasets_metadata'].get(dataset, {}).get(dataset_variant)
            if not metadata:
            ...
            if not check_s3_file_exists(bucket, train_key):
                try:
                    ...
                    # Notare che ora execute restituisce una tupla, prendiamo solo le righe
                    calculated_train_rows, _ = execute_streaming_split(dataset, dataset_variant)
                except Exception as e:
    ...
    # In train+infer mode we send only the final message, not the intermediate one
    if job_data.get('mode') == 'train':
        client_response_queue = config["sqs_queues"].get("client_response")
        if client_response_queue:
            response_payload = {
                "job_id": job_id, "target_model": job_id, "mode": "train",
...
def process_bulk_inference(job_data, job_id):
    """Gestisce la logica di valutazione su dataset massivi (Test Set)."""
    dataset = job_data['dataset']
    dataset_variant = job_data.get('dataset_variant', '1M')
...
def process_realtime_inference(job_data, job_id):
    """Gestisce le predizioni in tempo reale (Single Inference)."""
    dataset = job_data['dataset']
    target_model = job_data['target_model']
        ...
        if 'Messages' in response:
            ...
            try:
                if mode == 'train':
                    ...
                    process_training(job_data, job_id)
                    # pass just created id to the bulk inference funct
